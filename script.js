...
rouletteNumbers.forEach((slot, index) => {
    ...
    el.style.transform = `rotate(${angle}deg) translate(130px) translate(-50%, -50%)`;
    el.style.top = '50%';
    el.style.left = '50%';
...
betForm.addEventListener('submit', function(e) {
    ...
    // 生成隨機旋轉角度
    const rotation = 360 * Math.floor(Math.random() * 10) + Math.floor(Math.random() * 360);
    ...
    setTimeout(() => {
        ...
        const actualRotation = (rotation - 90) % 360;
        const index = Math.floor((actualRotation / 360) * rouletteNumbers.length);
        const winningNumber = rouletteNumbers[(rouletteNumbers.length - index - 1) % rouletteNumbers.length].number;

        if (betType === 'red' || betType === 'black') {
            const winningColor = rouletteNumbers.find(num => num.number === winningNumber)?.color;
            ...
            if (winningNumber === '0' || winningNumber === '00') {
                ...
                if ((betType === 'even' && isEven) || (betType === 'odd' && !isEven)) {
                    ...
                    resultDiv.textContent = `You lost. The number was ${winningNumber}, which is ${isEven ? 'even' : 'odd'}.`;
                }
            }
            ...
            if (winningNumber === '0' || winningNumber === '00') {
                ...
                const num = parseInt(winningNumber);
                if ((betType === 'big' && num >= 19 && num <= 36) ||
                    (betType === 'small' && num >= 1 && num <= 18)) {
                    resultDiv.textContent = `You won! The number was ${winningNumber}, which is ${betType}.`;
                } else {
                    resultDiv.textContent = `You lost. The number was ${winningNumber}, which is ${num >= 19 && num <= 36 ? 'big' : 'small'}.`;
                }
            }
